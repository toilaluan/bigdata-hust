...
def weaviate_writer(df, epoch_id):
    ...
    .mode("append").save()
    # Transform the DataFrame df as needed
    # transformed_df = df

    # # Write the data to Weaviate
    # transformed_df.write.format("io.weaviate.spark.Weaviate").option(
    #     "batchSize", 16
    # ).option("scheme", "http").option("host", "localhost:8080").option(
    #     "id", "uuid"
    # ).option("className", "Estate").option("vector", "vector").mode("append").save()
